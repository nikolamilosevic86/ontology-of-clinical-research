		...
		try {
			...
			for (OWLOntology ont : imports){
				...
				if (root.equals("research")){
					...
					resetSeed();
					Set<OWLProperty> props = ont.getPropertiesInSignature();
					for (OWLEntity prop : props) {
						System.out.println(prop.getIRI());
						//Call renameURI(..)						
						setLabelProp(manager, df, ont, prop, labelProp);
						if (prop.getIRI().toString().contains(root)) {
							IRI newNameIRI = getNewNameIRI(ont.getOntologyID().getOntologyIRI());
							System.out.println("Renaming "+prop.getIRI().toString()+ " to "+newNameIRI);
							renameURI(prop, newNameIRI, manager);
						}
					}
